 ...
 */
export function arraysAreEqual<T extends Array<unknown>>(
	arr1: T,
	arr2: T,
): boolean {
...
export function getNextMatch(
	...
	if (lowerSearch.endsWith(" ")) {
		 ...
		 */
		const spacedMatches = values.filter((value) =>
			value.toLowerCase().startsWith(lowerSearch),
		);
